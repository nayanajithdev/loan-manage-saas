...
require_permission('collections.loan_records_edit', 'pages/loans.php');
...
function collection_update_snapshot_row(array $snapshot): array
{
...
function collection_update_delete_generated_installments(PDO $pdo, int $loanId, array $groups): void
{
    ...
    foreach ($groups as $group) {
        ...
        foreach ($snapshots as $snapshot) {
            ...
            $row = collection_update_snapshot_row($snapshot);
...
function collection_update_restore_snapshots(PDO $pdo, int $loanId, array $snapshots): void
{
    if ($snapshots === []) {
    ...
    foreach ($snapshots as $snapshot) {
        ...
        if ((int) ($row['loan_id'] ?? 0) !== $loanId) {
            throw new RuntimeException('Cannot edit because an installment snapshot belongs to a different loan.');
        }
...
function collection_update_next_installment_id(PDO $pdo, int $loanId): int
{
    $stmt = $pdo->prepare(
        "SELECT id
...
if ($loanId <= 0 || $collectionId <= 0 || $amount <= 0 || $collectedOn === '') {
    set_flash('error', 'Collection, amount and date are required.');
    redirect($returnTo);
}

$dateObj = DateTimeImmutable::createFromFormat('Y-m-d', $collectedOn);
if (!$dateObj || $dateObj->format('Y-m-d') !== $collectedOn) {
    set_flash('error', 'Invalid collection date.');
    redirect($returnTo);
}

if ($collectedOn > today()) {
    set_flash('error', 'Collection date cannot be in the future.');
    redirect($returnTo);
}
...
try {
    ...
    foreach ($groupsToReplay as $group) {
        ...
        if ((string) ($group['payment_ref'] ?? '') === '' || $groupSnapshots === []) {
            throw new RuntimeException('This collection cannot be edited because it or a later collection is missing safe replay data.');
        }
    }
    ...
    foreach ($groupsToReplay as &$group) {
        ...
        $group['amount'] = $amount;
        $group['collected_on'] = $collectedOn;
        $group['method'] = $method;
        $group['note'] = $note;
    }
    unset($group);
    ...
    collection_update_restore_snapshots($pdo, $loanId, $selectedSnapshots);

    foreach ($groupsToReplay as $group) {
        $nextInstallmentId = collection_update_next_installment_id($pdo, $loanId);
    ...
    log_activity($pdo, 'collection.updated', $currentUserName . ' updated collection for loan ' . (string) ($loan['loan_number'] ?? ('#' . $loanId)) . '.', [
        'loan_id' => $loanId,
        'collection_id' => $collectionId,
        'payment_ref' => (string) $selectedGroup['payment_ref'],
        'amount' => $amount,
        'collected_on' => $collectedOn,
        'replayed_group_count' => count($groupsToReplay),
    ]);
    ...
    log_activity($pdo, 'collection.update_failed', 'Collection update failed: ' . $e->getMessage(), [
        'loan_id' => $loanId,
        'collection_id' => $collectionId,
    ]);
